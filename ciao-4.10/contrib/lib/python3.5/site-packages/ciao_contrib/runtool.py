...
def _add_history_via_crates( infile, tool, param_names, param_values ):
    import pycrates as pyc
    import history as pyh
    
    outfile = pyc.read_file(infile, mode="rw")
    histnum = outfile.get_key("HISTNUM")
    if histnum is None:
        histnum = pyc.CrateKey()
        histnum.name = "HISTNUM"
        histnum.value = 1
        outfile.add_key(histnum)
    
    startat = histnum.value
    if startat is None:
        startat = 1
    else:
        # crates give np.int() which history doesn't like
        startat = int(startat)+1 


    # Because start at is diff for each outfile, need to do this
    # for all file in outfile stack individually.
    history = pyh.HistoryRecord(tool=tool, param_names=param_names,
                        param_values=param_values, asc_start=startat)
    history_len = len(history.as_ASC_FITS().split("\n"))
    
    pyc.add_history( outfile, history)
    histnum.value = startat + history_len
...
def add_tool_history(infiles, toolname, params,
    ...
    The history module is new in CIAO 4.11.
    """
    ...
    for infile in infiles:
        ...
        _add_history_via_crates(infile, toolname, param_names, param_values)
        




def orig_add_tool_history(infiles, toolname, params,
                     toolversion=None, tmpdir=None):
    """Add a CIAO history block to each of the files in infiles saying
    that toolname was run with the given set of parameters (a
    key/value dictionary). If infiles is a single string then it is
...
parinfo['celldetect'] = {
    ...
    'opt': [ParValue("expstk","f","list of exposure map files",None),ParValue("regfile","f","ASCII regions file",'none'),ParValue("clobber","b","Overwrite exiting outputs?",False),ParValue("thresh","r","Source threshold",3),ParValue("findpeaks","b","Find local peaks?",True),ParValue("centroid","b","Compute source centroids?",True),ParValue("ellsigma","r","Size of output source ellipses (in sigmas)",3),ParValue("expratio","r","cutoff ratio for source cell exposure variation",0),ParValue("fixedcell","i","Fixed cell size to use (0 for variable cell)",0),ParValue("xoffset","i","Offset of x axis from data center",None),ParValue("yoffset","i","Offset of y axis from data center",None),ParValue("eband","r","Energy band",1.4967),ParValue("eenergy","r","Encircled energy of PSF",0.8),ParValue("psftable","f","Table of PSF size data",'${ASCDS_CALIB}/psfsize20010416.fits'),ParValue("cellfile","f","Output cell size image stack name",None),ParRange("maxlogicalwindow","i","maximum size for the logical window",32768,0,None),ParValue("bkgfile","f","Background file name",None),ParValue("bkgvalue","r","Background count/pixel",0),ParValue("bkgerrvalue","r","Background error",0),ParValue("convolve","b","Use convolution?",False),ParValue("snrfile","f","SNR output file name (for convolution only)",None),ParRange("verbose","i","Log verbosity level",0,0,5),ParValue("log","b","Make a celldetect.log file?",False)],
    }
...
parinfo['destreak'] = {
    ...
    'opt': [ParValue("max","s","streak threshold  syntax:  m  OR  m:m:m:m",None),ParRange("max_rowloss_fraction","r","Maximum fraction of avg streaks/node/frame",5.0e-5,0.3e-4,1),ParValue("num_sigma","r","Sigma value for determining streak threshold",1.0),ParValue("filter","b","Discard tagged events",True),ParValue("mask","s","Filter to select candidate streak events",'[status=0,grade=0,2:4,6]'),ParValue("ccd_id","s","CCD ID to filter",'8'),ParValue("ccd_col","s","CCD ID column name",'ccd_id'),ParValue("node_col","s","Node ID column name ('none' for single node)",'node_id'),ParValue("exptime","r","frame time (s) (reads EXPTIME if no pos. value given)",-1),ParValue("countfile","f","filename for event row-count distribution",None),ParValue("fracfile","f","filename for cumulative streak contam function",None),ParValue("timefile","f","filename for exposure time lost per row",None),ParRange("verbose","i","Debug Level(0-5)",0,0,5),ParValue("clobber","b","Clobber existing file",False)],
    }
...
parinfo['fullgarf'] = {
    ...
    'opt': [ParValue("dafile","s","NONE, CALDB, or name of ACIS dead-area calibration file",'CALDB'),ParValue("osipfile","s","NONE or Name of fits file with order sorting info",'CALDB'),ParValue("clobber","b","Clobber existing output files? This is passed to ALL child processes.",False),ParRange("verbose","i","Control the level of diagnostic output. 0=>least.",0,0,5)],
    }
...
parinfo['hrc_process_events'] = {
    ...
    'opt': [ParValue("alignmentfile","f","sim/fam alignment file ( NONE | none | <filename>)",')acaofffile'),ParValue("obsfile","f","obs.par file for output file keywords ( NONE | none | <filename>)",'NONE'),ParValue("geompar","f","Parameter file for Pixlib Geometry files",'geom'),ParValue("do_ratio","b","perform ratio validity checks",True),ParValue("do_amp_sf_cor","b","perform amp_sf correction (y/n) ?",True),ParValue("gainfile","f","gain correction image file ( NONE | none | <filename>)",'CALDB'),ParValue("ADCfile","f","ADC correction table file ( NONE | none | <filename>)",'NONE'),ParValue("degapfile","f","degap factors (NONE | none | COEFF | <filename>)",'CALDB'),ParValue("hypfile","f","Hyperbolic test coefficients file ( NONE | none | <filename>)",'CALDB'),ParValue("ampsfcorfile","f","caldb file for amp_sf_correction( NONE | none | <filename>)",'CALDB'),ParValue("tapfile","f","tap ring test coefficients file ( NONE | none | <filename>)",'CALDB'),ParValue("ampsatfile","f","ADC saturation test file ( NONE | none | <filename>)",'CALDB'),ParValue("evtflatfile","f","Event flatness test file ( NONE | none | <filename>)",'CALDB'),ParValue("badfile","f","output level 1 bad event file",'lev1_bad_evts.fits'),ParValue("logfile","f","debug log file (STDOUT | stdout | <filename>)",'stdout'),ParValue("instrume","s","hrc instrument- used for parameter file",'hrc-i'),ParValue("eventdef","s","output format definition",')stdlev1'),ParValue("badeventdef","s","output format definition",')badlev1'),ParRange("grid_ratio","r","charge ratio",0.5,0.0,1.0),ParRange("pha_ratio","r","pha ratio",0.5,0.0,1.0),ParValue("wire_charge","i","turn on center wire test (-1=off,0=on)",0),ParValue("cfu1","r","u axis 1st order cor. factor",1.0),ParValue("cfu2","r","u axis 2nd order cor. factor",0.0),ParValue("cfv1","r","v axis 1st order cor. factor",1.0),ParValue("cfv2","r","v axis 2nd order cor. factor",0.0),ParValue("time_offset","r","Offset to add to event time field to synch w/ fam data",0.0),ParValue("amp_gain","r","amp gain",75.0),ParRange("rand_seed","i","random seed (for pixlib), 0 = use time dependent seed",1,0,32767),ParSet("rand_pix_size","r","pixel randomization width (-size..+size), 0.0=no randomization",0.0,[0.0,0.5]),ParValue("tstart","s","header key containing default time value (HSI)",'TSTART'),ParValue("tstop","s","header key containing time of last event",'TSTOP'),ParSet("start","s","start transformations at",'coarse',["coarse","chip","tdet"]),ParSet("stop","s","end transformations at",'sky',["none","chip","tdet","det","sky"]),ParValue("stdlev1","s","event format definition string",'{d:time,s:crsv,s:crsu,s:amp_sf,s:av1,s:av2,s:av3,s:au1,s:au2,s:au3,l:raw,s:chip,l:tdet,f:det,f:sky,s:pha,s:pi,s:sumamps,s:chip_id,x:status}'),ParValue("badlev1","s","event format definition string",'{d:time,s:crsu,s:crsv,s:au1,s:au2,s:au3,s:av1,s:av2,s:av3,s:pha}'),ParValue("hsilev1","s","event format definition string",'{d:time,s:crsu,s:crsv,s:au1,s:au2,s:au3,s:av1,s:av2,s:av3,s:chipx,s:chipy,s:tdetx,s:tdety,s:x,s:y,l:fpz,s:pha,s:vstat,s:estat}'),ParValue("simlev1","s","sim event definition string",'{l:tick,i:scifr,i:mjf,s:mnf,s:evtctr,s:crsu,s:crsv,s:au1,s:au2,s:au3,s:av1,s:av2,s:av3,s:tdetx,s:tdety,s:pha,s:vstat,s:estat}'),ParValue("fltlev1","s","event format definition string",'{d:time,s:crsv,s:crsu,s:amp_sf,s:av1,s:av2,s:av3,s:au1,s:au2,s:au3,s:chipx,s:chipy,l:tdetx,l:tdety,s:detx,s:dety,s:x,s:y,s:pha,s:sumamps,s:chip_id,l:status}'),ParValue("clobber","b","Overwrite output event file if it already exists?",False),ParRange("verbose","i","level of debug detail (0=none, 5=most)",0,0,5)],
    }
...
parinfo['psfsize_srcs'] = {
    ...
    'opt': [ParValue("energy","s","Monochromatic energy to use in PSF lookup [keV]",'broad'),ParRange("ecf","r","Encircled counts fraction (psf fraction)",0.9,0,1.0),ParValue("psffile","f","REEF Caldb filename",'CALDB'),ParRange("verbose","i","Tool chatter level",0,0,5),ParValue("clobber","b","Removing existing files?",False)],
    }
...
parinfo['skyfov'] = {
    ...
    'opt': [ParValue("logfile","f","Name of log file",'STDOUT'),ParValue("kernel","s","Output file format, ASCII or FITS",'FITS'),ParValue("aspect","f","Aspect file or a stack of aspect file list",None),ParValue("mskfile","f","Mask file to retain active windows of chips",None),ParValue("geompar","f","Pixlib geometry parameter file",'geom'),ParValue("clobber","b","Overwrite existing outfile (yes|no)?",False),ParRange("verbose","i","Verbosity level (0 = no display)",0,0,5)],
    }
...
parinfo['tg_resolve_events'] = {
    ...
    'opt': [ParValue("alignmentfile","f","Input sim offset file",')acaofffile'),ParValue("logfile","f","Output log (NONE|<filename>|stdout)",'stdout'),ParValue("osipfile","f","Lookup table for order resolving (for acis data only)",'CALDB'),ParRange("osort_lo","r","Order-sorting lower bound fraction; order > m - osort_lo",0.3,0,0.5),ParRange("osort_hi","r","Order-sorting high bound fraction; order <= m + osort_hi",0.3,0,0.5),ParSet("grating_obs","s","Observed grating type (header_value|HETG|HEG|MEG|LETG)",'header_value',["HETG","HEG","MEG","LETG","header_value","HEADER_VALUE"]),ParSet("detector","s","Detector type: ACIS | HRC-I | HRC-S | header_value",'header_value',["ACIS","HRC-I","HRC-S","header_value","HEADER_VALUE"]),ParRange("energy_lo_adj","r","Lower Energy limit factor",1.0,0,10),ParRange("energy_hi_adj","r","Upper Energy limit factor",1.0,0,10),ParValue("time_offset","r","Offset to add to event time to synch w/ alignment data",0),ParRange("rand_seed","i","Random seed (for pixlib), 0 = use time dependent seed",1,0,32767),ParSet("rand_pix_size","r","pixel randomization width (-size..+size), 0.0 = no randomization",0,[0,0.5]),ParValue("eventdef","s","Output format definition",')stdlev1_ACIS'),ParValue("stdlev1","s","",')eventdef'),ParValue("stdlev1_ACIS","s","ACIS event format definition string",'{d:time,i:expno,f:rd,s:chip,s:tdet,f:det,f:sky,s:ccd_id,l:pha,s:pi,f:energy,s:grade,s:fltgrade,s:node_id,s:tg_m,f:tg_lam,f:tg_mlam,s:tg_srcid,s:tg_part,s:tg_smap,x:status}'),ParValue("stdlev1_HRC","s","HRC event format definition string",'{d:time,f:rd,s:chip,l:tdet,f:det,f:sky,s:chip_id,s:pha,s:pi,s:tg_m,f:tg_lam,f:tg_mlam,s:tg_srcid,s:tg_part,s:tg_smap,x:status}'),ParValue("cclev1a","s","Lev1.5 CC faint event format definition string",'{d:time,d:time_ro,l:expno,s:ccd_id,s:node_id,s:chip,f:chipy_tg,f:chipy_zo,s:tdet,f:det,f:sky,f:sky_1d,s:phas,l:pha,l:pha_ro,f:energy,l:pi,s:fltgrade,s:grade,f:rd,s:tg_m,f:tg_lam,f:tg_mlam,s:tg_srcid,s:tg_part,s:tg_smap,x:status}'),ParValue("ccgrdlev1a","s","lev1.5 cc graded event format definition string",'{d:time,d:time_ro,l:expno,s:ccd_id,s:node_id,s:chip,f:chipy_tg,f:chipy_zo,s:tdet,f:det,f:sky,f:sky_1d,l:pha,l:pha_ro,s:corn_pha,f:energy,l:pi,s:fltgrade,s:grade,f:rd,s:tg_m,f:tg_lam,f:tg_mlam,s:tg_srcid,s:tg_part,s:tg_smap,x:status}'),ParValue("geompar","f","Parameter file for Pixlib Geometry files",'geom'),ParRange("verbose","i","Verbosity level of detail (0=none, 5=most)",0,0,5),ParValue("clobber","b","Clobber outfile if it already exists?",False)],
    }
...
parinfo['tgdetect'] = {
    ...
    'opt': [ParValue("temproot","s","Path and root file name to be given to temporary files",None),ParValue("keeptemp","b","Keep temporary files?",False),ParValue("keepexit","b","Keep exit status file?",False),ParValue("zo_pos_x","s","Center GZO filter sky X position (default=pixel(ra_nom))",'default'),ParValue("zo_pos_y","s","Center GZO filter sky Y position (default=pixel(dec_nom))",'default'),ParValue("zo_sz_filt_x","s","Size of GZO filter in X pixels (ACIS=400; HRC=1800)",'default'),ParValue("zo_sz_filt_y","s","Size of GZO filter in Y pixels (ACIS=400; HRC=1800)",'default'),ParRange("snr_thresh","r","SNR threshold to select the detected sources",40,3,10000),ParValue("expstk","f","list of exposure map files",'none'),ParRange("thresh","r","celldetect source threshold",3,3,10000),ParValue("ellsigma","r","Size of output source ellipses (in sigmas)",3.0),ParValue("expratio","r","cutoff ratio for source cell exposure variation",0),ParValue("findpeaks","b","find local peaks for celldetect",True),ParValue("celldetect_log","b","make a celldetect log file?",False),ParValue("psftable","f","table of PSF size data, for celldetect",'${ASCDS_CALIB}/psfsize20010416.fits'),ParRange("fixedcell","i","celldetect fixed cell size to use",15,0,999),ParRange("fixedcell_cc_mode","i","celldetect fixed cell size to use for CC mode ACIS data",15,0,999),ParValue("bkgfile","f","background file, for celldetect",'none'),ParRange("bkgvalue","r","background count/pixel, for celldetect",0,0,None),ParRange("bkgerrvalue","r","background error, for celldetect",0,0,None),ParRange("eband","r","energy band, for celldetect",1.4967,0,None),ParRange("eenergy","r","encircled energy of PSF, for celldetect",0.8,0,None),ParValue("snrfile","f","celldetect snr output file (for convolution only)",'none'),ParValue("convolve","b","use convolutions for celldetect",False),ParRange("xoffset","i","celldetect offset of x axis from optical axis",None,"INDEF","INDEF"),ParRange("yoffset","i","celldetect offset of y axis from optical axis",None,"INDEF","INDEF"),ParValue("cellfile","f","output cell size image file",'none'),ParValue("centroid","b","compute source centroids in celldetection?",True),ParRange("snr_ratio_limit","r","Value of SNR ratio to use as lower limit, for tgidselectsrc",1.0,0.0,1.0),ParValue("setsrcid","b","Set src ids in output file?, for tgidselectsrc",True),ParRange("max_separation","r","Maximum allowed separation (arcsec) for sources to match, for tgmatchsrc",3,0,20),ParValue("clobber","b","OK to overwrite existing output file(s)?",False),ParRange("verbose","i","Verbosity level (0 = no display)",0,0,5)],
    }
...
parinfo['tgidselectsrc'] = {
    'istool': True,
    'req': [ParValue("infile","f","Name of input src file",None),ParValue("outfile","f","Name of output filtered src file",None),ParRange("snr_ratio_limit","r","Value of SNR ratio to use as lower limit",1.0,0.0,1.0)],
